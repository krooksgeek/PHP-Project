...
//echo "Connected successfully";
  ...
  $sql = "SELECT * FROM `complaints` WHERE status = 'Pending' or status = 'On Progress'";
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    echo "<font color='cyan'>User Proof : <br></font></b>";
	
	echo '<img  src="'.$row['user_image'].'" width=500 height=300>';
    echo ""."<br>";
	echo "<font color='cyan'>Officer Proof : <br></font></b>";
	echo '<img  src="'.$row['off_image'].'" width=500 height=300>';
	
    // session_start();
    // $_SESSION['comid'] = $row['cid'];
    ?>
    
      <form method="post" action="/project/post_solution.php">
      <input type="hidden" name="comid" value= "<?php echo (isset($row["cid"]))?$row["cid"]:'';?>" readonly>
...
<br><br>
<button onclick="window.location.href='officer_home.php'" class="button button2">Back To Home</button>
<?php
  }
  else{
